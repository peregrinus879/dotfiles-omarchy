...
- The Makefile `TWIN_SPECS` files (nvim vault plugin specs, the `tdw` workspace function, `yazi.toml`) are byte-identical twins with EyrWSL; `make verify` fails on drift.
