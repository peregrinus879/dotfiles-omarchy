...
- `hdw` is added: the herdr counterpart of `dw`, one herdr workspace per project with the same layout (full agent tab; `$EDITOR` over a 25% shell in an editor+terminal tab), the same `cc|oc` single-agent choice and `-c` flag, and a root-collision guard backed by a label-to-root record under `~/.local/state/hdw/roots` (workspace ids recycle across server restarts, so the record keys on the label). Bare `hdw` refocuses; when the herdr server is down, `hdw` starts it headless and attaches, so one invocation works from a cold boot. Additive alongside Omarchy's `hdl`/`hds` pane layouts; sourced only when `herdr` is installed and not a twin (herdr is not on WSL).
