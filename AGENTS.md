...
- The Makefile `TWIN_SPECS` files (nvim vault plugin specs, the `trio` workspace function, `yazi.toml`) are byte-identical twins with `dotfiles-wsl`; `make verify` fails on drift.
