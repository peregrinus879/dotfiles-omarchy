...
- Claude Code launches use the supported maximum effort, `--effort max`. The `claude` alias applies it to every interactive launch, including `cx` and `tdl`-launched AIs; scripts, hooks, and the pinned `spar-claude` bridge stay plain because they do not expand aliases. This intentionally overrides the `effortLevel` pin EyrAgents tracks for interactive Omarchy sessions.
