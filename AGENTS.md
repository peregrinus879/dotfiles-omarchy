...
- The `hypr` package carries personal Hyprland overrides only, loaded after the Omarchy defaults through the Omarchy-owned `~/.config/hypr/hyprland.lua` require chain: `bindings.lua` (the twelve default web-app bindings retired via `hl.unbind`, the personal `SUPER ALT` set and AppImages added), `monitors.lua` (display values plus the NVIDIA client env workaround), and `input.lua` (us/ara layouts, natural scrolling); no defaults are replicated (deviations documented in `DEVIATIONS.md`).
